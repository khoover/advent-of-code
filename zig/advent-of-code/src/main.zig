...
const ProblemState = struct {
    iter: SharedIterator(u32),
    solution_1: std.atomic.Value(u32),
    solution_2: std.atomic.Value(u32)
};
...
fn check_chunk(
    comptime md5_chunk_size: usize,
    message_buffers: *[md5_chunk_size][64]u8,
    prefix_len: usize,
    base: u32,
    state: *ProblemState
) void {
...
inline fn encode_base(buf: []u8, base: u32) usize {
    var cur = base;
    var i = buf.len - 1;
    while (cur > 0) : ({cur = @divTrunc(cur, 10); i -= 1;}) {
...
inline fn write_offsets(
    comptime N: usize,
    buf: *[N][64]u8,
    offset_start_idx: usize,
    chunk_offset: usize
) void {
...
pub fn main(init: std.process.Init) !void {
    var state = ProblemState{
        .iter=.init(1000, 1000),
        .solution_1=.init(0xFFFFFFFF),
        .solution_2=.init(0xFFFFFFFF)
    };
    ...
    const core_count = try std.Thread.getCpuCount() / 2;
